...
def test_compute_detectors_for_fixed_radius_with_database(
    init_plaquettes: Plaquettes, memory_plaquettes: Plaquettes, k: int
) -> None:
    """Test compute_detectors_for_fixed_radius with database in both parallel and non-parallel modes."""
    d = 2 * k + 1
    template = FixedTemplate(
        [[0 if (i + j) % 2 == 0 else 1 for j in range(d)] for i in range(d)]
    )
    ...
    # Run with database, non-parallel
    detectors = compute_detectors_for_fixed_radius(
        [template, template], 
        k, 
        [init_plaquettes, memory_plaquettes],
    ...
    # Database should be populated after first run
    initial_db_size = len(database)
    assert initial_db_size > 0
    
    # Clear the database and run with parallel=True
    database_parallel = DetectorDatabase()
    assert len(database_parallel) == 0
    
    # Run with database, parallel mode
    parallel_detectors = compute_detectors_for_fixed_radius(
        [template, template], 
        k, 
        [init_plaquettes, memory_plaquettes],
        database=database_parallel,
        parallel=True
    )
    
    # Database should be populated with the same number of entries
    assert len(database_parallel) == initial_db_size
    assert len(parallel_detectors) == len(detectors)
    
    # Run both again but using only_use_database=True
    reused_detectors = compute_detectors_for_fixed_radius(
        [template, template], 
        k, 
        [init_plaquettes, memory_plaquettes],
        database=database,
        only_use_database=True,
        parallel=False
    )
    
    reused_parallel_detectors = compute_detectors_for_fixed_radius(
        [template, template], 
        k, 
        [init_plaquettes, memory_plaquettes],
        database=database_parallel,
    ...
    # Should get the same results in both cases
    assert len(reused_detectors) == len(detectors)
    assert len(reused_parallel_detectors) == len(parallel_detectors)


@pytest.mark.parametrize("parallel", [False, True])
def test_compute_detectors_only_use_database_missing_entry(
    init_plaquettes: Plaquettes, parallel: bool
) -> None:
    """Test that only_use_database=True raises an exception when database doesn't contain needed entries."""
    k = 1
    d = 2 * k + 1
    template = FixedTemplate(
        [[0 if (i + j) % 2 == 0 else 1 for j in range(d)] for i in range(d)]
    ...
    # Should raise exception when database is empty but only_use_database=True
    with pytest.raises(TQECException):
        compute_detectors_for_fixed_radius(
            [template], 
            k, 
