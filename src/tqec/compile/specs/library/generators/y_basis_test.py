...
from tqec.compile.specs.library.generators.y_basis import (
    _make_qubit_patch,
    _make_y_basis_initialization_chunks,
    _make_y_basis_measurement_chunks,
    _standard_surface_code_chunk,
)
...
def make_y_memory_experiment(
    ...
    y_init_chunks = _make_y_basis_initialization_chunks(
        distance, top_boundary_basis, convention, padding_rounds
    )
    y_meas_chunks = _make_y_basis_measurement_chunks(
        distance, top_boundary_basis, convention, padding_rounds
    )
    ...
    memory_patch = _make_qubit_patch(distance, top_boundary_basis, convention)
    memory_round = _standard_surface_code_chunk(memory_patch, obs=qubit_obs)
