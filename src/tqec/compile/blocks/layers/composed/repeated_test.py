...
def test_scalable_timesteps(plaquette_layer: PlaquetteLayer) -> None:
    assert RepeatedLayer(plaquette_layer, LinearFunction(0, 1)).scalable_timesteps == LinearFunction(0, 1)
    assert RepeatedLayer(plaquette_layer, LinearFunction(1, 90)).scalable_timesteps == LinearFunction(1, 90)
...
def test_with_spatial_borders_trimmed(borders: tuple[SpatialBlockBorder, ...], plaquette_layer: PlaquetteLayer) -> None:
    ...
    expected_plaquette_indices = all_indices - frozenset(
        itertools.chain.from_iterable(
            frozenset(plaquette_layer.template.get_border_indices(border.to_template_border())) for border in borders
        )
    )
    trimmed_layer = layer.with_spatial_borders_trimmed(borders)
    assert isinstance(trimmed_layer.internal_layer, PlaquetteLayer)
    assert frozenset(trimmed_layer.internal_layer.plaquettes.collection.keys()) == expected_plaquette_indices
...
def test_to_sequenced_layer_with_schedule(
    ...
    with pytest.raises(
        ...
        repeated_layer.to_sequenced_layer_with_schedule((LinearFunction(0, 2), LinearFunction(6, 4)))
...
def test_with_temporal_borders_replaced_none(plaquette_layer: PlaquetteLayer) -> None:
    ...
    assert layer.with_temporal_borders_replaced({TemporalBlockBorder.Z_NEGATIVE: None}) == RepeatedLayer(
        plaquette_layer, LinearFunction(2, 1)
    )
    assert layer.with_temporal_borders_replaced({TemporalBlockBorder.Z_POSITIVE: None}) == RepeatedLayer(
        plaquette_layer, LinearFunction(2, 1)
    )
    ...
    assert layer.with_temporal_borders_replaced({}) == layer
    assert layer.with_temporal_borders_replaced({TemporalBlockBorder.Z_NEGATIVE: None}) == plaquette_layer
    assert layer.with_temporal_borders_replaced({TemporalBlockBorder.Z_POSITIVE: None}) == plaquette_layer
    assert (
...
def test_with_temporal_borders_replaced(
    ...
    for replacement in [plaquette_layer, plaquette_layer2, raw_circuit_layer]:
        assert layer.with_temporal_borders_replaced({TemporalBlockBorder.Z_NEGATIVE: replacement}) == SequencedLayers(
            [replacement, plaquette_layer]
        )
        assert layer.with_temporal_borders_replaced({TemporalBlockBorder.Z_POSITIVE: replacement}) == SequencedLayers(
            [plaquette_layer, replacement]
        )
        assert layer.with_temporal_borders_replaced(
            {
                TemporalBlockBorder.Z_NEGATIVE: replacement,
