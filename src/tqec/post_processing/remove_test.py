...
def test_remove_leading_trailing_tick() -> None:
    ...
    assert remove_empty_moments(c, remove_trailing_tick=False, remove_leading_tick=False) == stim.Circuit(
        "TICK\nH 0\nTICK"
    )


def test_within_circuit() -> None:
    assert remove_empty_moments(stim.Circuit("TICK\nH 0\nTICK\nTICK\nH 1")) == stim.Circuit("H 0\nTICK\nH 1")


def test_repeat_block_conventions() -> None:
    assert remove_empty_moments(stim.Circuit("H 0\nTICK\nREPEAT 5 {\nH 0\nTICK\n}\nTICK\nH 0")) == stim.Circuit(
        "H 0\nREPEAT 5 {\nTICK\nH 0\n}\nTICK\nH 0"
    )
