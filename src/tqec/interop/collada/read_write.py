...
# IMPORTER FOR DICTIONARY-BASED SPACE-TIME DIAGRAMS USING WEBGL-COMPATIBLE METHODS
def read_from_lattice_dicts(
    lattice_nodes: dict[int, tuple[tuple[int, int, int], str]],
    lattice_edges: dict[tuple[int, int], str],
    graph_name: str = "",
) -> BlockGraph:
    """Construct a :class:`.BlockGraph` from a space-time diagram given as a 3D-graph dictionary.
        ...
        for pos, cube_kind in parsed_cubes:
            if isinstance(cube_kind, YHalfCube):
                block_graph.add_cube(
                    _int_position_before_scale(
                        _offset_y_cube_position(pos, pipe_length), pipe_length
                    ),
                    cube_kind,
                )
            else:
