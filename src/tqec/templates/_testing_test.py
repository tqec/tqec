...
def test_instantiation() -> None:
    ...
    numpy.testing.assert_array_equal(FixedTemplate([[1]]).instantiate(plaquette_indices=[1, 2]), [[2]])
...
def test_shape() -> None:
    assert FixedTemplate([[0]]).scalable_shape == PlaquetteScalable2D(LinearFunction(0, 1), LinearFunction(0, 1))
